...
void addCPforData(complex32* pAfterIFFT,complex32* pBeforeAddWin,int N_SYM, int symbol){
   ...
   // for(i=0; i<N_STS; i++){
        memcpy(pBeforeAddWin+symbol*(DftSize+CpLen), pAfterIFFT+DftSize-CpLen, CpLen*sizeof(complex32));
        memcpy(pBeforeAddWin+symbol*(DftSize+CpLen)+CpLen, pAfterIFFT, DftSize*sizeof(complex32));
  //  }
/*
     //add cp
     int i=0;
